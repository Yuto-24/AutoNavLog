...
- RJFMのUMK/OMARU座標識別は表示名でなく座標で行う。一致座標に明示的なPoint Placemarkが
  ある場合は、そのPoint名と`IMPORTED`由来を物理スロットに残す。RJFM出発で同じ座標の
  LineString区切り名を置換する場合だけ、その名称は後続の通常座標へ元の順序で再配置する。
  Point名と同じ区切り名は1件だけ消費し、同名の別の区切り名は維持する。RJFM到着では
  明示Point以外のLineString名を移動しない。この処理はRouteNodeや座標を追加・移動せず、
  RJFM以外の名称規則へ適用しない。
