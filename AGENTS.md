...
- Every Issue change must synchronize the version in `pyproject.toml`, `web/package.json`, both root and `packages[""]` entries in `web/package-lock.json`, `README.md`, and `CHANGELOG.md`; record the Issue and summary in the changelog. Run `pytest tests/unit/test_version.py` and `npm --prefix web run typecheck`. Do not change the separately managed `src/autonavlog/version.py` or `jma-msm-wind` version.
