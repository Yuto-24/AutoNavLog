...
class MsmMetarTrendQnhProvider:
    ...
    def _now(self) -> datetime:
        return self._metar._now_utc()

    def _lookups(self, now: datetime) -> dict[str, Any]:
        return self._metar._lookup_metars(DEFAULT_METAR_STATIONS, now)
    ...
    def _estimate_qnh(
        ...
        target = self._delegate.query_batch(forecast_run_id, (target_request,))[0]
            ...
            if age < -_MAXIMUM_FUTURE_CLOCK_SKEW:
                ...
                if baseline_mslp is not None:
                    ...
                    if 800 < estimate < 1100:
                        provenance = self._metar._observation_provenance(
                            observation,
                            age,
        ...
        return WeatherResult(
            ...
            values={
                ...
                "forecast_time_utc": request.valid_time_utc.isoformat(),
                "msm_target_mslp_hpa": target_mslp,
                "fallback_reason": fallback_reason or "METAR_QNH_UNAVAILABLE",
            },
            ...
            metadata={
                "provider": "msm-metar-trend",
                "qnh_method": "MSM_MSLP_ONLY",
                "metar_fallback_reason": fallback_reason,
