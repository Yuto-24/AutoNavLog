...
- RJFMからUMK/RCAまでの`ETE`と燃料は、同じ気温補正を適用したPOH上昇表の
  ...
  未丸め合計をPOH値と一致させます。
- 主経路の直線Legから求めた`DIST`、`TC`、`WCA`、`MH`、`GS`はそのまま表示します。
  したがってこの例外の`CLIMB`行では、意図的に`DIST / GS != ETE`となります。
...
- RWY09はMC 092°を1,000 ft MSLまで維持し、左へ45°変針してMC 047°とします。
  ...
  直線、必要な回数の左完全旋回、左部分旋回、UMKへの接線直線を組み合わせ、
