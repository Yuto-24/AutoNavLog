...
| 降下・到着 | CAC-REV19 8-(2)「3. 航法諸元」、8-(3)「5.(3)〜(4)」「6.(3)〜(4)」 | 宮崎DescentはCruise airspeed・500 fpm。降下TASは巡航CASを用いる。EOCから目視位置通報点までは中間高度の予想風、目視位置通報点から目的空港まではCAS 121 kt・無風でETEを算出する | 500 fpm、降下CASの考え方、到着CAS 121 kt・無風は裏付け済み。1000 fpmは利用者が明示選択する代替計画値で、規程上の標準値としての裏付けはない。規程は降下前にFuel Flowを18 GPHへsetする操作も記載するが、計画燃料は下記12 GPH |
...
| EOC距離 | CAC-REV19 8-(4)「9.(2) EOC」および8-(3)「6.(3)」 | 降下TASと予想風からGSを求め（風を予想しない場合はTAS）、500 fpmで求めた降下ETEからEOC位置を算出する | 500 fpmの原則は裏付け済み。1000 fpm時の同じ逆算は利用者選択を適用する実装Policy。規程本文9.(2)の参照番号表記と実装の区間分割詳細はGolden照合が必要 |
...
- NAV LOG計算で`PA = MSL`とするプロジェクト定義。
- 上昇表の500 ft線形補間、表端から500 ft以内の高度外挿、POHの温度補正との対応。
- 巡航表のPWR→ISA偏差→高度という補間順序、0.1単位のties-to-even、および各cornerの
  65% PWRが掲載範囲外のとき最寄り2行から線形外挿するAutoNavLog Policy。高度・ISA偏差の
  外挿は禁止する。規程が要求する「計画に近い条件のうち不利」を満たすことのGolden照合。
...
- EOCの追加1分、選択降下率、DESCENT基準Legで不足したときに経路始点側のLegを候補へ加え、候補開始
  高度からVREPまでを連続降下として再計算する実装詳細、0.5 NM未満の変針点snap、および
