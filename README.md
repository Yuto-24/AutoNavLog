...
- 表示中のNAV LOG計算結果をJSONファイルへダウンロード、またはクリップボードへコピーする
...
NAV LOGのJSON出力は表示中の計算結果と目的地風を含みます。未計算の入力変更は含まず、
Projectのbackup / restore形式ではありません。
...
「KMLを貼り付け」を押すと、クリップボードのKMLを直接読み込んで経路候補を表示します。
ブラウザから許可や貼り付けの操作を求められた場合は、その案内に従ってください。
読み取りが許可されない環境やテキストが空の場合は、手動で貼り付ける入力欄が開きます。
確認をキャンセルした場合も同じ入力欄からファイル選択へ切り替えられます。
KMLを読み込めなかった場合は、入力欄で内容を修正して再試行できます。
取り込み後は地図と記載順を確認してから経路を確定してください。
