...
- NAV LOGのVOR/DME基準局候補に天草（AKE）が含まれていなかった不具合を修正しました。手動選択・自動候補選定とradial / 距離表示に利用できます。

### 開発者向け

- 既存のAIPサイクルのRJDA AD 2.19から、AKEの座標・周波数・VOR局偏差と出典を追加しました。
- AKEの収録値、自動候補選定、手動選択とTO座標に対する計算の回帰検証を追加しました。
...
- MSMの微風で風向がない場合に、TAS・ETE・燃料計算が停止する不具合を修正しました。
- RJFM固定RCAの上昇計算が、UMK以降の不要な上昇高度の気象に依存して失敗する不具合を修正しました。POH上昇時間・燃料と区間ごとの表示風は維持します。

### 開発者向け

- Legacy / Local共通MSMアダプターで有効なCALMを計算用0 ktへ正規化し、元値をメタデータに保持します。
- 検証済みRJFM固定RCAを距離探索より先に適用し、境界以降のCLIMB気象要求を省きます。
...
- 再読み込み前の計算や別タブの未保存編集が、保存済みデータを後から上書きしないようにしました。
